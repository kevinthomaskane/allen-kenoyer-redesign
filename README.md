...
`content/`, `scripts/`, `docs/`, `demo/`, and `.agents/` are listed in `.vercelignore` and never reach the Vercel build environment. Code in `src/` must not import from them — see [`docs/dev-guide.md`](./docs/dev-guide.md) → _Import boundaries_.
