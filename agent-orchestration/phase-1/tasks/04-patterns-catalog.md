...
- `src/lib/patterns.test.ts` asserts catalog-construction invariants per [ADR-0017](../../decisions/0017-content-modeling-patterns-catalog.md) (uniqueness on `(category, number)`, not bare `number`, since five numbers appear in more than one category as distinct designs).
