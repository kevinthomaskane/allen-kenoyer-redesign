...
1. **Linter:** ESLint, pinned to the **9.x maintenance line** (current: `eslint@9.39.4`). See "ESLint version line" below for why 9.x and not 10.x.
...
ESLint is pinned to the 9.x maintenance line, not the 10.x release line. ESLint 10 is incompatible with the React/Next.js ecosystem in practice: `eslint-config-next@16.2.6` (matching Next.js 16.2.6) transitively pulls `eslint-plugin-react@7.37.5`, which still calls `context.getFilename()` — removed in ESLint 10, with no published fix or ETA. The 9.x line (`9.39.4`) is fully supported and is what the plugin ecosystem targets in 2026. Revisit once `eslint-plugin-react` ships ESLint 10 support.
